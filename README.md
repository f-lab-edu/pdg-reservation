...
- 보상 트랜잭션(Compensation) 설계: DB 영속화 단계에서 예외 발생 시, 이미 승인된 PG 결제를 즉시 강제 취소하는 보상 로직을 구현하여 '결제 완료-예약 누락'이라는 데이터 부정합 상태를 0%로 제어했습니다.
...
- 
### 🔥 Issue 7: 비동기 이벤트를 활용한 도메인 결합도 분리 및 리뷰 시스템
리뷰 등록/삭제 시 발생하는 부수적인 로직(평점 재계산 등)을 메인 트랜잭션에서 분리하여 시스템 확장성을 확보했습니다.
- 비동기 평점 업데이트 (EDA): @TransactionalEventListener를 도입하여 리뷰 작성 로직과 무거운 집계(평균 평점) 로직을 분리, 사용자 응답 속도를 개선했습니다.
- 데이터 정합성 정책: 캐시 정합성 관리의 복잡도를 낮추기 위해 '수정 불가(Immutable)' 정책을 채택하고, 데이터 이력 보존을 위해 Soft Delete를 적용하여 운영 안정성을 높였습니다.
...
- [x] Soft Delete 및 비동기 평점 갱신 로직을 포함한 리뷰 시스템 구축
- [ ] Redis 캐싱을 적용한 숙소 상세 및 리뷰 조회 성능 고도화 예정
