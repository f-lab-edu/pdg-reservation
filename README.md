...
- 데이터 정합성 및 멱등성 정책: '수정 불가(Immutable)' 정책과 Soft Delete를 적용해 데이터 신뢰성을 높였으며, event_log 테이블을 활용해 비동기 이벤트 재처리에 따른 중복 반영을 방지(멱등성)했습니다.
...
- [x] AOP @Order 제어를 통한 분산 락-트랜잭션 간 정합성 보호 및 캐시 스탬피드 방어
